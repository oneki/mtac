...
INSERT INTO resource (urn, label, pub, tenant_id, link_id, schema_id, content, created_at, updated_at, created_by, updated_by)
VALUES ('urn:root:schema:tenant.root', 'tenant.root', true, null, null, 1, to_json('{"name":"root","parents":[{"label":"tenant","id":2,"schema":"schema"}]}'::JSON), NOW(), NOW(), 'root@local', 'root@local');
...
INSERT INTO resource (urn, label, pub, tenant_id, link_id, schema_id, content, created_at, updated_at, created_by, updated_by)
VALUES ('urn:root:schema:iam.identity.user', 'iam.identity.user', true, 4, null, 1, to_json('{"name":"User","parents":[{"label":"iam.identity","id":6,"schema":"schema"}]}'::JSON), NOW(), NOW(), 'root@local', 'root@local');

-- Create the schema iam.identity.group
INSERT INTO resource (urn, label, pub, tenant_id, link_id, schema_id, content, created_at, updated_at, created_by, updated_by)
VALUES ('urn:root:schema:iam.identity.group', 'iam.identity.group', true, 4, null, 1, to_json('{"name":"Group","parents":[{"label":"iam.identity","id":6,"schema":"schema"}]}'::JSON), NOW(), NOW(), 'root@local', 'root@local');
