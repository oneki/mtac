...
public class FilterCriteria {
  ...
  public FilterCriteria(ResourceDesc resourceDesc, Operator operator, String field, String value, boolean negation,
      boolean join, boolean relation) {
    ...
    if (field.equals("uid") || field.endsWith(".uid")) {
      if (operator == Operator.IN) {
        // if the operator is IN, we convert ids to integer because ids are sometimes
        // used for joining tables
        List<String> values = Arrays.asList(value.split(","));
        value = values.stream().map(v -> "" + Resource.fromUid(v)).collect(Collectors.joining(","));
      } else {
        value = "" + Resource.fromUid(value);
      }
    }
    // replace uid or .uid by id or .id
    if (field.equals("uid")) {
      field = "id";
  ...
  public Map<String, String> toSql(Map<String, String> tableAliases, String namedParameter, Object value) {
    ...
    // leftJoinTable: " LEFT JOIN json_array_elements((r.content->'memberOf')::json)
    // jae_table1 ON TRUE",
    // table: ""
    // }
    Map<String, String> leftOperand = SqlUtils.getLeftOperand(resourceDesc, field, tableAliases,
    ...
    if (value == null) {
      ...
      // Example: we need to filter all the groups of a user whose description is
      // "foo"
      // We've generated a temporary table "table1" which contains only all the groups
      // whose description is "foo"
      // We then need to make a join between r.content.memberOf[].id and table1.id
      where += "EXISTS (SELECT 1 FROM " + ((String) value).split("\\.")[0] + " WHERE " + leftOperand.get("operand")
          + " = " + value + ")";
  ...
  public Object getSqlValue() {
    ...
    Set<Operator> dateOperators = Set.of(Operator.IN, Operator.GREATER, Operator.GREATER_OR_EQUALS, Operator.LESSER,
        Operator.LESSER_OR_EQUALS, Operator.EQUALS);
    Set<Operator> numberOperators = Set.of(Operator.GREATER, Operator.GREATER_OR_EQUALS, Operator.LESSER,
        ...
        return values.stream().map(v -> {
          try {
            ...
            throw new BusinessException(BusinessException.BUSINESS_INVALID_SYNTAX,
                v + " is not a correct datetime (format is yyyy-MM-dd HH:mm:ss)");
          }
    ...
    if (dateFields.contains(sqlField)) {
      if (!dateOperators.contains(operator)) {
        throw new BusinessException(BusinessException.BUSINESS_INVALID_FIELD,
            "Operator " + operator + " is not supported for the field " + sqlField);
      }
      try {
        return new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(value);
      } catch (ParseException e) {
        throw new BusinessException(BusinessException.BUSINESS_INVALID_SYNTAX,
            value + " is not a correct date (format is yyyy-MM-dd HH:mm:ss)");
      }
    }
    ...
    if (operator == Operator.EQUALS) {
      switch (value.toLowerCase()) {
        case "true":
          // When using a GIN index (the one used for JSONB columns), the boolean must be
          // a string (if the operator is '=')
          return "true";
        case "false":
          return "false";
        default:
          // the value is always converted ton a string exception if it's the id (which is
    ...
    if (operator == Operator.IS) {
      switch (value.toLowerCase()) {
        // When the operator is "IS", the value must be null, true or false (so not a
        // string)
        case "null":
          return null;
        case "true":
